...
import { Package, MapPin, DollarSign, Send, MessageSquare, Info, Flame, Rocket, Globe, MapPinCheck, AlertTriangle, Calendar, History, Briefcase } from "lucide-react";
...
const DashboardMontador = () => {
  ...
  const fetchMyBids = async () => {
    if (!user) return;
    const { data } = await supabase.from("bids").select("*").eq("montador_id", user.id);
  ...
  const myAcceptedBidOrderIds = new Set(myBids.filter(b => b.accepted).map(b => b.order_id));

  // Mural: pending orders I haven't bid on yet
  const muralOrders = orders.filter(o => o.status === "pendente" && !myBidOrderIds.has(o.id));
  const filteredMural = montadorCity && !showAllRegion
    ? muralOrders.filter(o => o.city === montadorCity)
    : muralOrders;

  // Agenda: orders where my bid was accepted and service is active
  const activeStatuses = ["aceito", "pago", "em_andamento", "desmontagem_confirmada"];
  const agendaOrders = orders.filter(o => myAcceptedBidOrderIds.has(o.id) && activeStatuses.includes(o.status));

  // Histórico: completed/released orders
  const historicoStatuses = ["aguardando_liberacao", "concluido"];
  const historicoOrders = orders.filter(o => myAcceptedBidOrderIds.has(o.id) && historicoStatuses.includes(o.status));

  // Also show orders where I bid but wasn't accepted yet or was accepted with com_lance
  const pendingBidOrders = orders.filter(o => myBidOrderIds.has(o.id) && !myAcceptedBidOrderIds.has(o.id) && o.status === "com_lance");
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <Tabs defaultValue="mural" className="w-full">
          ...
          <TabsContent value="mural" className="space-y-4">
            ...
            {/* Pending bids (orders I already bid on) */}
            {pendingBidOrders.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-medium text-muted-foreground">Lances enviados (aguardando resposta)</p>
                {pendingBidOrders.map(order => (
                  <Card key={order.id} className="border-primary/20">
                    <CardContent className="py-3 flex items-center justify-between">
                      <div>
                        <p className="font-medium text-sm">{order.title}</p>
                        <p className="text-xs text-muted-foreground">{order.furniture_type} · {order.city || "Sem cidade"}</p>
                      </div>
                      <Badge className={statusColors["com_lance"]}>{statusLabels["com_lance"]}</Badge>
                    </CardContent>
                  </Card>
                ))}
              </div>
            ...
            {filteredMural.length === 0 ? (
              ...
              <div className="grid gap-4">
                {filteredMural.map((order) => {
                  ...
                  // Only show neighborhood, never full address
                  const neighborhood = order.address.split(",")[0];
                    ...
                    <Card key={order.id} className={`overflow-hidden ${isUrgent ? "border-destructive/60 shadow-lg shadow-destructive/10" : ""}`}>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <p className="text-sm text-muted-foreground flex items-center gap-1">
                          <MapPin className="h-3.5 w-3.5" /> {neighborhood} · {order.city || "Sem cidade"}
                        </p>

                        {needsWallMount && (
                          <Alert className="border-warning/50 bg-warning/10">
                            <AlertTriangle className="h-5 w-5 text-warning" />
                            <AlertTitle className="text-warning font-bold">⚠️ Requer furação de parede</AlertTitle>
                        ...
                        {isUrgent && (
                          <div className="rounded-lg bg-destructive/10 border border-destructive/30 p-3 flex items-start gap-2">
                            ...
                            <div>
                              <p className="text-sm font-semibold text-destructive">🚀 Pedido urgente — Taxa Zero!</p>
                              <p className="text-xs text-muted-foreground mt-0.5">Você recebe o valor total do seu lance.</p>
                            </div>
                          </div>
                        )}
                  ...
                  return (
                    <Card key={order.id} className="overflow-hidden border-primary/30">
                      ...
                      <CardContent className="space-y-3">
                        ...
                        {/* Full address only shown here after formal hiring */}
                        {isPaid ? (
                          <div className="rounded-lg bg-success/10 border border-success/30 p-3">
                            <p className="text-sm font-medium flex items-center gap-1">
                              <MapPin className="h-4 w-4 text-success" /> Endereço de Montagem:
                            </p>
                        ...
                        {acceptedBid && (
                          <p className="text-sm text-muted-foreground">
                            Valor do lance: <strong className="text-primary">R$ {acceptedBid.amount.toFixed(2)}</strong>
                          </p>
                        )}
                      ...
                      <CardContent className="py-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{order.furniture_type} · {new Date(order.created_at).toLocaleDateString("pt-BR")}</p>
                            {acceptedBid && (
                              <p className="text-sm mt-1">
                                Valor: <strong className="text-success">R$ {calcMontadorReceives(acceptedBid.amount, isUrgent).toFixed(2)}</strong>
                                {isUrgent && <span className="text-xs text-destructive ml-1">(Taxa Zero)</span>}
                              </p>
                            )}
                          </div>
                          <Badge className={statusColors[order.status]}>
